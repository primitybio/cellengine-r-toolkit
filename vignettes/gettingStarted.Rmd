...
```R
gate = getGate("57c4d7d4e69a1e890c4d8d2f", byName("gate_name"))
```
...
```R
lookup = createLookup("57c4d7d4e69a1e890c4d8d2f")
gate = lookup("gates", "gate_name")
population = lookup("populations", "population_name")
```

Looking up a resource by name requires two API calls, and so is slightly slower
